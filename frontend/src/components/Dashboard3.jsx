...
const Dashboard3 = () => {
  ...
  // Fetch data from API
  const getData = () => {
    AxiosInstance.get("dashboar3Data/")
      .then((res) => {
        setdashboar1Data(res.data);
        setFilteredData(res.data); // Initially, filtered data is the same as fetched data
      })
      .catch((error) => {
  ...
  // Handle filter data (from FilterBox)
  ...
  return (
    <div>
      {/* FilterBox is outside the chart */}
      <FilterBox onFilterChange={handleFilterChange} />

      {/* Only render charts if filteredData is not empty */}
